...
-- 1) UN COUNT PARA EL NUMERO DE PELICULAS CON CIERTO NOMBRE (DESDE TABLA TEMPORAL, CONSULTAR DESDE LA TABLA PELICULAS)
...
-- 2) SALE LA CANTIDAD DE RENTAS POR CLIENTE (DESDE TABLA TEMPORAL, CONSULTAR DESDE LA TABLA CLIENTE)
...
-- 5)
SELECT DIRECCION.Pais, CLIENTE.Nombre, CLIENTE.Apellido, (CLIENTE.Rentas*100/(SELECT SUM(Rentas) FROM CLIENTE))as porcentaje 
...
-- 6)
SELECT (SELECT COUNT(*) FROM CLIENTE) AS TOTAL_CLIENTES, D.Pais, D.Ciudad, (
	SELECT COUNT(*)
	FROM DIRECCION INNER JOIN CLIENTE ON DIRECCION.idDireccion = CLIENTE.idDireccion
